...
class PPEComplianceRecord(models.Model):
    ...
    Links directly to a Worker + PPECatalog item so that compliance
    checks can be recorded without requiring a separate PPEItem
    assignment record.  The legacy ``ppe_item`` FK is kept nullable
    ...
    # ── Primary references (new direct links) ─────────────────
    worker = models.ForeignKey(
