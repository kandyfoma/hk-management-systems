...
export function SpirometryListScreen() {
  ...
  const [selectedItem, setSelectedItem] = useState<SpirometryResult | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const api = ApiService.getInstance();
      const response = await api.post('/occupational-health/spirometry-results/', {
        worker_id: selectedWorker.id,
        test_date: formData.test_date,
        fev1: parseFloat(formData.fev1),
        fvc: parseFloat(formData.fvc),
        fev1_fvc_ratio: formData.fev1_fvc_ratio ? parseFloat(formData.fev1_fvc_ratio) : null,
  ...
  const handleOpenEdit = (item: SpirometryResult) => {
    setSelectedItem(item);
    setEditFormData({
      test_date: item.test_date || '',
      fev1: item.fev1?.toString() || '',
      fvc: item.fvc?.toString() || '',
      fev1_fvc_ratio: item.fev1_fvc_ratio?.toString() || '',
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const patchData = {
        ...
        fev1: parseFloat(editFormData.fev1) || 0,
        fvc: parseFloat(editFormData.fvc) || 0,
        fev1_fvc_ratio: editFormData.fev1_fvc_ratio ? parseFloat(editFormData.fev1_fvc_ratio) : null,
        interpretation: editFormData.interpretation,
        notes: editFormData.notes,
      };

      const response = await api.patch(`/occupational-health/spirometry-results/${selectedItem.id}/`, patchData);
      if (response.success) {
        setResults(results.map(r => r.id === selectedItem.id ? { ...r, ...patchData } : r));
  ...
  const handleDelete = async (item: SpirometryResult) => {
        ...
        <View style={styles.contentSection}>
          {loading ? (
              ...
              <View key={result.id} style={[styles.resultCard, shadows.sm]}>
                <View style={styles.resultCardLeft}>
                  <View style={[styles.statusIcon, { backgroundColor: getStatusColor(result.status) + '20' }]}>
                    <Ionicons name="leaf" size={24} color={getStatusColor(result.status)} />
                  </View>
                </View>
                <View style={styles.resultCardCenter}>
                  ...
                  <Text style={styles.resultValues}>FEV1: {result.fev1}% | FVC: {result.fvc}% | Ratio: {result.fev1_fvc_ratio}%</Text>
                </View>
            ...
            <View style={styles.emptyState}>
              <Ionicons name="leaf-outline" size={48} color={colors.textSecondary + '40'} />
        ...
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { maxHeight: '90%' }]}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Ajouter une spirométrie</Text>
            ...
            <ScrollView style={styles.formSection}>
              ...
              <TextInput style={styles.input} placeholder="0-100" keyboardType="decimal-pad" value={formData.fev1} onChangeText={(text) => setFormData({ ...formData, fev1: text })} placeholderTextColor={colors.textSecondary} />
              <Text style={styles.formLabel}>FVC (%)</Text>
              <TextInput style={styles.input} placeholder="0-100" keyboardType="decimal-pad" value={formData.fvc} onChangeText={(text) => setFormData({ ...formData, fvc: text })} placeholderTextColor={colors.textSecondary} />
              <Text style={styles.formLabel}>FEV1/FVC Ratio</Text>
              <TextInput style={styles.input} placeholder="0-100" keyboardType="decimal-pad" value={formData.fev1_fvc_ratio} onChangeText={(text) => setFormData({ ...formData, fev1_fvc_ratio: text })} placeholderTextColor={colors.textSecondary} />
              <Text style={styles.formLabel}>Interprétation</Text>
              <TextInput style={styles.input} placeholder="Ex: Normal, Restrictif..." value={formData.interpretation} onChangeText={(text) => setFormData({ ...formData, interpretation: text })} placeholderTextColor={colors.textSecondary} />
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.formSection}>
              ...
              <Text style={styles.formLabel}>FEV1 (%)</Text>
              <TextInput style={styles.input} keyboardType="decimal-pad" value={editFormData.fev1} onChangeText={(text) => setEditFormData({ ...editFormData, fev1: text })} />
              <Text style={styles.formLabel}>FVC (%)</Text>
              <TextInput style={styles.input} keyboardType="decimal-pad" value={editFormData.fvc} onChangeText={(text) => setEditFormData({ ...editFormData, fvc: text })} />
              <Text style={styles.formLabel}>FEV1/FVC Ratio</Text>
              <TextInput style={styles.input} keyboardType="decimal-pad" value={editFormData.fev1_fvc_ratio} onChangeText={(text) => setEditFormData({ ...editFormData, fev1_fvc_ratio: text })} />
              <Text style={styles.formLabel}>Interprétation</Text>
              <TextInput style={styles.input} value={editFormData.interpretation} onChangeText={(text) => setEditFormData({ ...editFormData, interpretation: text })} />
              <Text style={styles.formLabel}>Notes</Text>
